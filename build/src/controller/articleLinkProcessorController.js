...
const processArticleFromLink = async (req, res) => {
  try {
    ...
    if (!articleUrl) return res.status(400).json({
      error: "Vui lòng cung cấp liên kết bài báo!"
    });
    let htmlContent;
    try {
    ...
    const rawDate = cleanText($(".list-group-item.date-published").text() || $("meta[property='article:published_time']").attr("content") || $(".pub-date, .date").text());
    const publish_date = formatDate(rawDate);
    const abstract = cleanText($(".abstract, .summary").text());
    const description = cleanText($("meta[property='og:description']").attr("content") || "Không có mô tả");
    const keywords = cleanText($("meta[name='keywords']").attr("content") || $(".keywords").text()).replace(/^Keywords:\s*/i, "");
