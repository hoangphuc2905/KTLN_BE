...
const authController = {
  ...
  registerStudent: async (req, res) => {
    try {
      ...
      if (existingStudent) {
        return res.status(400).json({
          message: "Mã số sinh viên hoặc email đã tồn tại"
        });
      }
      const student = new Student({
        ...
        degree: "None",
        role: "student",
        avatar: "https://i.pinimg.com/1200x/bc/43/98/bc439871417621836a0eeea768d60944.jpg"
      });
      await student.save();
  ...
  approveStudent: async (req, res) => {
    try {
      ...
      if (!account) {
        return res.status(404).json({
          message: "Account not found for this student"
        });
      }
      ...
      const mailOptions = {
        ...
        to: student.email,
        subject: "Hệ thống quản lý các bài báo nghiên cứu khoa học của sinh viên và giảng viên trường Đại học Công Nghiệp TPHCM",
