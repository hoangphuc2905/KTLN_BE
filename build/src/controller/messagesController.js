...
const messagesController = {
  ...
  getAllMessages: async (req, res) => {
    try {
      const messages = await Messages.find().populate("sender_id") // Populate thông tin người gửi
  ...
  getMessagesByReceiverId: async (req, res) => {
    try {
      const {
        receiverId
      } = req.params; // Lấy receiverId từ params
  ...
  markMessageAsRead: async (req, res) => {
    try {
      const {
        id
      } = req.params; // Lấy ID của thông báo từ URL params
