...
const lecturerController = {
  ...
  assignRole: async (req, res) => {
    try {
      ...
      } else if (admin.roles.some(role => role.role_name === "deputy_head_of_department")) {
        if (admin.department.toString() !== lecturer.department.toString()) {
          return res.status(403).json({
            message: "You can only change roles within your department"
          });
        }
        if (lecturer.roles.some(role => role.role_name === "head_of_department")) {
          return res.status(403).json({
            message: "You cannot change the role of the head of department"
          });
        }
      } else if (admin.roles.some(role => role.role_name === "department_in_charge")) {
        if (admin.department.toString() !== lecturer.department.toString()) {
          return res.status(403).json({
            message: "You can only change roles within your department"
          });
        }
        if (lecturer.roles.some(role => role.role_name === "head_of_department" || role.role_name === "deputy_head_of_department")) {
          return res.status(403).json({
            message: "You cannot change the role of the head of department or deputy head of department"
          });
        }
      } else {
        return res.status(403).json({
      ...
      if (newRole === "head_of_department") {
        ...
        if (deputyCount >= 1) {
          return res.status(400).json({
            message: "There can only be one deputy head of department in a department"
          });
        }
        ...
        if (inChargeCount >= 3) {
          return res.status(400).json({
            message: "There can only be up to three department in charge in a department"
          });
        }
      }
  ...
  deleteLecturerById: async (req, res) => {
    try {
      ...
      res.status(200).json({
        message: "Lecturer and account deleted successfully"
      });
    } catch (error) {
      res.status(500).json({
        message: error.message
