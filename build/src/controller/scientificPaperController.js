...
} = require("./fileCloudinaryController");
const {
  generateEmbedding
...
const scientificPaperController = {
  createScientificPaper: async (req, res) => {
    ...
    try {
      ...
      const lastPaper = await ScientificPaper.findOne().sort({
        paper_id: -1
      });
      ...
      const roleNames = ["head_of_department", "deputy_head_of_department", "department_in_charge"]; // Các vai trò cần gửi thông báo
      ...
      for (const lecturer of lecturers) {
        const messageData = {
          ...
          // ID của bài báo
          content: `Có một bài báo mới cần duyệt: ${scientificPaper.title}`,
          // Nội dung thông báo
          isread: false,
          time: new Date()
        };
        ...
        } // Giả lập res (không cần trả về gì)
        );
      }
  ...
  getAllScientificPapers: async (req, res) => {
    try {
      const {
        academicYear
      } = req.query; // Get academicYear from query string
      ...
      const scientificPapers = await ScientificPaper.find(filter).populate("article_type").populate("article_group").populate({
        path: "author",
        populate: {
          path: "work_unit_id",
          model: "WorkUnit"
      ...
      res.status(200).json({
        message: academicYear ? `Scientific papers for academic year ${academicYear}` : "All scientific papers retrieved successfully",
  ...
  getScientificPaperById: async (req, res) => {
    try {
      const scientificPaper = await ScientificPaper.findById(req.params.id).populate("article_type").populate("article_group").populate({
        path: "author",
        populate: {
          path: "work_unit_id",
          model: "WorkUnit"
      ...
      if (!scientificPaper) {
        return res.status(404).json({
          message: "ScientificPaper not found"
        });
      }
  ...
  getScientificPapersByAuthorId: async (req, res) => {
    try {
      const {
        userId
      } = req.params; // Lấy `userId` từ URL params
      const {
        academicYear
      ...
      if (!authors || authors.length === 0) {
        return res.status(404).json({
          message: "No authors found with this user_id."
        });
      }
      ...
      if (!scientificPapers || scientificPapers.length === 0) {
        return res.status(404).json({
          message: "No scientific papers found for this user_id."
        });
      }
      res.status(200).json({
        message: academicYear ? `Scientific papers for user ${userId} in academic year ${academicYear}` : `All scientific papers for user ${userId}`,
        academicYear: academicYear || "All",
        scientificPapers
      });
    } catch (error) {
  ...
  getScientificPapersByDepartment: async (req, res) => {
    try {
      ...
      res.status(200).json({
        message: academicYear ? `Scientific papers for department ${department} in academic year ${academicYear}` : `All scientific papers for department ${department}`,
        academicYear: academicYear || "All",
        scientificPapers
      });
    } catch (error) {
  ...
  updateScientificPaperStatus: async (req, res) => {
    try {
      ...
      const scientificPaper = await ScientificPaper.findByIdAndUpdate(id, {
        status
      }, {
        new: true,
        runValidators: true
  ...
  updateScientificPaperById: async (req, res) => {
    ...
    try {
      ...
      const updatedPaper = await ScientificPaper.findByIdAndUpdate(id, updatedPaperData, {
        new: true,
        runValidators: true,
        session
      });
      ...
      const roleNames = ["head_of_department", "deputy_head_of_department", "department_in_charge"];
      const roles = await Role.find({
        role_name: {
          $in: roleNames
        }
  ...
  getTop5NewestScientificPapers: async (req, res) => {
    try {
      const topPapers = await ScientificPaper.find({
        status: "approved"
      }).sort({
        createdAt: -1
      }).limit(5).populate("article_type").populate("article_group").populate({
        path: "author",
        populate: {
          path: "work_unit_id",
          model: "WorkUnit"
    ...
    try {
      ...
      const topPapers = await ScientificPaper.aggregate([{
        ...
        $match: {
          ...
          // Chỉ lấy bài có lượt xem > 0
          {
            downloadCount: {
              $gt: 0
            }
          } // Hoặc lượt tải xuống > 0
          ]
        }
      }, {
        $sort: {
