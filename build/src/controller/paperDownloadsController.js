...
const createPaperDownloadLimiter = rateLimit({
  ...
  max: 5,
  message: {
    message: "Bạn đã tải xuống quá nhiều lần trong thời gian ngắn. Vui lòng thử lại sau."
  }
});
const paperDownloadsController = {
  ...
  getAllPaperDownloadsByUser: async (req, res) => {
    try {
      ...
      // Sắp xếp theo thời gian tải xuống (mới nhất trước)
      {
        $group: {
          _id: "$paper_id",
          // Nhóm theo paper_id
      ...
      if (!populatedDownloads || populatedDownloads.length === 0) {
        return res.status(404).json({
          message: "No paper downloads found for this user"
        });
      }
