...
const paperCollectionController = {
  createCollection: async (req, res) => {
    try {
      const { name, user_type, user_id } = req.body;
      ...
      const collection = new PaperCollection({ name, user_type, user_id });
  ...
  getCollectionsByUserId: async (req, res) => {
    try {
      ...
      const collections = await PaperCollection.find({ user_id }).populate({
        path: "papers",
        populate: [
          { path: "article_type" },
          { path: "article_group" },
  ...
  addPaperToCollection: async (req, res) => {
    try {
      const { collection_id, paper_id } = req.body;
  ...
  removePaperFromCollection: async (req, res) => {
    try {
      ...
      collection.papers = collection.papers.filter(
        (id) => id.toString() !== paper_id
      );
      await collection.save();
