  ...
  getTotalPointByAuthorId: async (req, res) => {
    try {
      ...
      const result = await ScientificPaper.aggregate([
        ...
        {
          $unwind: "$authorDetails", // Tách từng phần tử trong mảng `authorDetails`
        },
        {
          $group: {
            _id: null,
            totalPoints: { $sum: "$authorDetails.point" }, // Tính tổng điểm đóng góp
          },
        },
      ]);
  ...
  getTop3PapersByAuthorId: async (req, res) => {
    try {
      ...
      const result = await ScientificPaper.aggregate([
        ...
        {
          $addFields: {
            viewCount: { $size: "$viewDetails" }, // Đếm số lượt xem
            downloadCount: { $size: "$downloadDetails" }, // Đếm số lượt tải xuống
            authorDetails: {
              $filter: {
                input: "$authorDetails", // Lọc danh sách tác giả
                as: "author",
