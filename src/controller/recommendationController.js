...
exports.getRecommendationsByUserHistory = async (req, res) => {
  try {
    ...
    const downloads = await PaperDownload.find({ user_id: userId });
    const views = await PaperView.find({ user_id: userId });

    const interactedPaperIds = [
      ...new Set([
        ...downloads.map((d) => d.paper_id.toString()),
        ...views.map((v) => v.paper_id.toString()),
      ]),
    ];

    if (interactedPaperIds.length === 0)
    ...
    if (viewedPapers.length === 0)
      return res.status(200).json({ message: "No embeddings found", data: [] });
