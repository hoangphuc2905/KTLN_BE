...
exports.calculateScoreFromInput = async (req, res) => {
  try {
    const { createdAt, doi, featured, article_group, authors } = req.body;

    const publishDate = new Date(createdAt);
    const formula = await ScoringFormula.findOne({
      startDate: { $lte: publishDate },
      endDate: { $gte: publishDate },
      ...
      for (const item of formula.formula) {
        ...
        switch (attribute.name) {
          ...
          case "institution_count":
            const count = Array.isArray(author.institutions)
    ...
    const totalScore = Object.values(authorScores).reduce(
